...
export class TwitchUserTrigger implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Twitch User Trigger',
		name: 'twitchUserTrigger',
		icon: { light: 'file:twitch.svg', dark: 'file:twitch.dark.svg' },
		group: ['trigger'],
		version: 1,
		subtitle: '={{$parameter["event"]}}',
		description: 'Listen to Twitch EventSub notifications via Webhook (User Access Token events only)',
		defaults: {
			name: 'Twitch User Trigger',
		},
		usableAsTool: true,
		inputs: [],
		...
		properties: userTriggerProperties,
	};
	...
	webhookMethods = {
		default: {
			...
			async create(this: IHookFunctions): Promise<boolean> {
				...
				// Pattern 6: user_id only (user.whisper.message for User token)
				if (event === 'user.whisper.message') {
